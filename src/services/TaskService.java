    ...
    public void paySideTask(Player player) {
        if (player.playerTask.sideTask.template != null) {
            if (player.playerTask.sideTask.isDone()) {
                ...
                boolean canGetBonusItem = false; // Điều kiện nhận vật phẩm phụ
                switch (player.playerTask.sideTask.level) {
                    ...
                    case ConstTask.HELL:
                        ...
                        canGetBonusItem = true; // Cấp địa ngục có thể nhận vật phẩm phụ
                        break;
                }

                // Kiểm tra đủ chỗ trống trong hành trang
                int itemsToReceive = Util.nextInt(minItems, maxItems);
                if (InventoryService.gI().getCountEmptyBag(player) >= itemsToReceive) {
                    // Nhận số lượng vật phẩm chính (ID: 457)
                    for (int i = 0; i < itemsToReceive; i++) {
                        Item item457 = ItemService.gI().createNewItem((short) 457);
                        InventoryService.gI().addItemBag(player, item457);
                    }
                    Service.gI().sendThongBao(player, "Bạn nhận được " + itemsToReceive + " thỏi vàng");
                    // Nếu cấp Địa Ngục, có 10% cơ hội nhận item phụ
                    if (canGetBonusItem) {
                        if (Util.isTrue(10, 100)) { // 10% nhận item 16
                            Item item16 = ItemService.gI().createNewItem((short) 16);
                            InventoryService.gI().addItemBag(player, item16);
                            Service.gI().sendThongBao(player, "Bạn nhận được vật phẩm đặc biệt");
                        }
                        if (Util.isTrue(10, 100)) { // 10% nhận item 674
                            Item item674 = ItemService.gI().createNewItem((short) 674);
                            InventoryService.gI().addItemBag(player, item674);
                            Service.gI().sendThongBao(player, "Bạn nhận được vật phẩm đặc biệt");
                        }
                    }
                    // Cập nhật kho đồ và vàng
                    InventoryService.gI().sendItemBag(player);
