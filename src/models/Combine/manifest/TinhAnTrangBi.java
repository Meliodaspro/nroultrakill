...
//            if (item.template.type <= 5) {
            if (item.template.id >= 650 && item.template.id <= 662 || item.template.id >= 555 && item.template.id <= 567|| item.template.id >= 1048 && item.template.id <= 1062) {
                return true;
            } else {
                return false;
            }
    ...
    public static void showInfoCombine(Player player) {
        if (InventoryService.gI().getCountEmptyBag(player) > 0) {
            if (player.combine.itemsCombine.size() == 2) {
                ...
                if (isTrangBiAn(item)) {
                    if (item != null && item.isNotNullItem() && dangusac != null && dangusac.isNotNullItem() && (dangusac.template.id == 1724 || dangusac.template.id == 1725 || dangusac.template.id == 1726) && dangusac.quantity >= 20) {
                        ...
                        npcSay += "|1|Con có muốn biến trang bị  " + item.template.name + " thành\n"
                                ...
                                + "|7|Cần 20 " + dangusac.template.name;
...
//                    InventoryService.gI().addItemBag(player, item);
                        InventoryService.gI().subQuantityItemsBag(player, dangusac, 20);
