...
public class NangCapBongTai {
    ...
    public static void showInfoCombine(Player player) {
        if (player.combine.itemsCombine.size() != 2) {
            Service.gI().sendDialogMessage(player, "Cần 1 bông tai cấp 1 và 999 mảnh vỡ bông tai.");
            return;
        }
        Item bongTai = null;
        Item manhVo = null;
        ...
        text.append(quantityManhVo >= 999 ? ConstFont.BOLD_BLUE : ConstFont.BOLD_RED).append("Cần 999 Mảnh vỡ bông tai\n");
        ...
        if (quantityManhVo < 999) {
            CombineService.gI().baHatMit.createOtherMenu(player, ConstNpc.IGNORE_MENU, text.toString(), "Còn thiếu\n" + (999 - quantityManhVo) + " Mảnh vỡ bông tai");
            return;
        }
    ...
    public static void nangCapBongTai(Player player) {
        ...
        int quantityManhVo = manhVo.getOptionParam(31);
        if (quantityManhVo < 999 || player.inventory.gold < 5_000_000 || player.inventory.getGemAndRuby() < 20) {
            return;
        }
        ...
        if (Util.isTrue(50, 100)) {
            ...
            InventoryService.gI().subParamItemsBag(player, 933, 31, 999);
        } else {
